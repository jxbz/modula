...
where :python:`learning_rate` is a :python:`float` and :python:`gradient` is the gradient of the loss function with respect to the :python:`weights` of the neural net. Of course, in practice, we may want to use additional tricks such as momentum, but let's ignore details like that for now.
